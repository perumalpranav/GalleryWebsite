...
> **Note**: The hosted version currently uses the legacy DynamoDB backend. The PostgreSQL migration is complete locally but online functionality is not yet live.
...
GalleryWebsite is an interactive and dynamic image gallery website. It enables users to view, search, and manage images along with their descriptions and associated keywords. This project integrates frontend and backend development, showcasing effective use of AWS services and a relational PostgreSQL database.
...
- **Database**: PostgreSQL (migrated from DynamoDB)
- **HTTP Requests**: Axios
- **Hosting**: AWS S3
...
3. **Database**:
   - PostgreSQL stores image metadata in a normalized relational schema across three tables: `images`, `keywords`, and `image_keywords`.
   - Replaces the previous DynamoDB implementation with structured querying, foreign key constraints, and transaction support.
   - **Note**: The PostgreSQL backend is fully implemented but not yet connected to the live hosted site.
...
- Connect the PostgreSQL backend to the live hosted site.
